<?get_header('taxonomy');?>
	...
	<main class="main">
			...
			<div class="catalog-list">
				...
				<ul>
					<?php foreach ($catlist as $cat) { ?>
						<li>
							<a href="<?= esc_url(get_term_link($cat, $cat->taxonomy))?>" class="link"><?= $cat->cat_name ?></a>
						</li>
					<?}?>
				</ul>
			</div>
		...
		<section class="section" id="sect1">
			<div class="container">
				<?
				$queried_object = get_term_by('slug', 'sistemy-izmerenija', 'products');
				$taxonomy = $queried_object->taxonomy;
				$term_id = $queried_object->term_id;
				$term_name = $queried_object->name;?>

				<h2 class="section-title h2 title c-title"><? single_term_title();?></h2>
